...
python3 tools/serve.py --dir build-web --port 8080
```

Then open `http://localhost:8080/moria-amiga.html` for the game, or
`amiga-gfx-test.html` for the frontend demo. `tools/serve.py` is
`http.server` plus a score history; a plain `python3 -m http.server` also
works, you just get no **Scores** tab. Opening the file directly will
not work: browsers refuse `.wasm` over `file://`.

The game page has four tabs — the game itself, a searchable **command
reference**, a **races and classes** table, and **Scores**. The two reference panels are
...
`#commands`, `#character` and `#scores`.

Every finished game is recorded — including a save-and-quit, which Moria
scores as `(saved)`. In the browser it is POSTed to the server that served the
page; natively it is appended to `scores.jsonl` beside the binary. Umoria's
own `scores.dat` is still written exactly as before; this is an addition.
...
| `tool-serve` | the score server: a posted game comes back, best first; junk types, unknown fields, malformed JSON and oversized bodies are refused; a torn last line costs only itself |
